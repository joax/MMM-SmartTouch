...
module.exports = NodeHelper.create({
  ...
  socketNotificationReceived: async function (notification, payload) {
    ...
    if (notification === "SET_ALL_GOVEE_DEVICES") {
      const { colorTemperature, brightness } = payload;
      
      console.log(`Setting all devices to: ${colorTemperature}K, ${brightness}%`);
      
      // Get all devices
      const devices = await this.getGoveeDevices();
      
      // Apply settings to each device
      const settingsPromises = devices.map(async device => {
        const results = {
          device: device.device,
          deviceName: device.deviceName,
          colorTempSuccess: false,
          brightnessSuccess: false
        };
        
        try {
          // Apply color temperature
          if (device.supportsColorTemp !== false) {
            const colorTempSuccess = await this.sendGoveeCommand(
              { name: "colorTem", value: colorTemperature }, 
              device.device, 
              device.model
            );
            results.colorTempSuccess = colorTempSuccess;
            
            if (colorTempSuccess) {
              console.log(`✓ Set ${device.deviceName} color temp to ${colorTemperature}K`);
            } else {
              console.log(`✗ Failed to set ${device.deviceName} color temp`);
            }
          } else {
            console.log(`⚠ ${device.deviceName} doesn't support color temperature`);
          }
          ...
          // Apply brightness
          const brightnessSuccess = await this.sendGoveeCommand(
            { name: "brightness", value: brightness }, 
            device.device, 
            device.model
          );
          results.brightnessSuccess = brightnessSuccess;
          
          if (brightnessSuccess) {
            console.log(`✓ Set ${device.deviceName} brightness to ${brightness}%`);
          } else {
            console.log(`✗ Failed to set ${device.deviceName} brightness`);
          }
          
        } catch (error) {
          console.error(`Error applying settings to ${device.deviceName}: ${error.message}`);
        }
        
        return results;
      });
      
      const allResults = await Promise.all(settingsPromises);
      
      // Count successes
      const colorTempSuccesses = allResults.filter(r => r.colorTempSuccess).length;
      const brightnessSuccesses = allResults.filter(r => r.brightnessSuccess).length;
      
      console.log(`Settings applied: ${colorTempSuccesses}/${devices.length} color temp, ${brightnessSuccesses}/${devices.length} brightness`);
      
      this.sendSocketNotification("ALL_GOVEE_DEVICES_SET", {
        colorTemperature: colorTemperature,
        brightness: brightness,
        results: allResults,
        totalDevices: devices.length,
        colorTempSuccesses: colorTempSuccesses,
        brightnessSuccesses: brightnessSuccesses
      });
    }
  },
});
